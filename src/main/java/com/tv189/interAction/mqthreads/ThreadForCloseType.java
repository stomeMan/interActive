...
@Component
public class ThreadForCloseType {
	@Autowired
	...
	public void run() {
		try {
			System.out.println(" Now is in ThreadForCloseType run method! ");
			final Integer threads = 2;
			List<KafkaStream<byte[], byte[]>> streams = consumer.pull(ParametersUtil.TOPIC_CLOSETYPE, threads);
			for (final KafkaStream<byte[], byte[]> kafkaStream : streams) {
				new Thread(new Runnable() {
					@Override
					public void run() {
						for (MessageAndMetadata<byte[], byte[]> messageAndMetadata : kafkaStream) {
							ParametersUtil.insertDB(JsonHelper.toJSONObject(new String(messageAndMetadata.message()), UserWinningCloseType.class), sqlSessionFactory);
						}
					}
				}).start();
			}
